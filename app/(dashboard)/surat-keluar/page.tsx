...
export default function SuratKeluarPage() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
          ...
          <table className="w-full text-left border-collapse min-w-[700px]">
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {filteredSurat.length === 0 ? (
                <tr key="empty-state">
                  <td colSpan={6} className="px-6 py-12 text-center text-slate-400">Tidak ada data surat keluar ditemukan.</td>
                </tr>
              ) : (
                filteredSurat.map((row, index) => (
                  <tr key={`${row.id}-${index}`} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 space-y-1">
                      ...
                      <p className="text-xs text-slate-400 mt-1">{row.tanggal_surat}</p>
                    </td>
                    <td className="px-6 py-4">
                      <p className="font-bold text-slate-700 line-clamp-1">{row.tujuan}</p>
                      <p className="text-xs text-slate-400">Pengonsep: {row.pembuat}</p>
                    </td>
                    <td className="px-6 py-4 flex items-center h-full">
                      <div className="truncate">
                        <p className="font-semibold text-slate-700 line-clamp-1">{row.perihal}</p>
                        <p className="text-xs text-indigo-600 font-mono">{row.no_surat || 'Belum Diberikan'}</p>
                      </div>
                      {row.file_surat && (
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-center space-x-2">
                        {!row.file_surat && (row.status === 'Draf' || row.status === 'Persetujuan') && (
                          <button onClick={() => router.push('/buat-surat')} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded" title="Edit di Generator">
        ...
        <form onSubmit={handleSaveNew} className="p-6 space-y-4 text-xs">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={newForm.no_surat} onChange={e => setNewForm({...newForm, no_surat: e.target.value})} placeholder="Kosongi jika draf" className="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div>
          ...
          <div>
            <label className="block font-semibold text-slate-600 mb-1">Tujuan / Nama Penerima</label>
            <input required type="text" value={newForm.tujuan} onChange={e => setNewForm({...newForm, tujuan: e.target.value})} placeholder="Nama Kantor atau Pejabat" className="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500" />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block font-semibold text-slate-600 mb-1">Pembuat / Konseptor</label>
              <input required type="text" value={newForm.pembuat} onChange={e => setNewForm({...newForm, pembuat: e.target.value})} className="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500" />
            </div>
          </div>

          <div>
            <label className="block font-semibold text-slate-600 mb-1">Status Alur Kerja Surat Keluar</label>
          ...
          <div>
            ...
            <textarea rows={3} value={newForm.isi_ringkas} onChange={e => setNewForm({...newForm, isi_ringkas: e.target.value})} placeholder="Intisari isi surat dinas..." className="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg text-sm resize-none focus:ring-2 focus:ring-indigo-500"></textarea>
          </div>
      ...
      <Modal isOpen={isDetailModalOpen} onClose={() => setIsDetailModalOpen(false)} title="Detail Informasi & Tindak Lanjut" maxWidth="max-w-lg" icon={<FileText className="text-indigo-600 w-5 h-5" />}>
        {selectedSuratData && (
          <div className="grid grid-cols-1 gap-6 p-6">
            ...
            <div className="space-y-4 text-xs">
              <div className="grid grid-cols-2 gap-4 pb-3 border-b border-slate-100">
                ...
                <div>
                  ...
                  <span className={`inline-block px-2 py-0.5 mt-1 rounded text-[10px] font-bold ${
                    selectedSuratData.sifat === 'Penting' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-700'
                  }`}>
              ...
              <div className="space-y-3 pb-4 border-b border-slate-100">
                <div>
                  ...
                  <p className="font-bold text-slate-800 text-sm font-mono mt-0.5">{selectedSuratData.no_surat || 'Belum diberikan'}</p>
                </div>

                <div>
                  <p className="text-[10px] text-slate-400 font-semibold uppercase">TUJUAN / PENERIMA</p>
                  <p className="font-bold text-slate-700 mt-0.5">{selectedSuratData.tujuan}</p>
                </div>
                ...
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-200/60">
                  ...
                  <p className="text-slate-700 mt-1 font-sans">{selectedSuratData.isi_ringkas || '-'}</p>
                </div>
              </div>
              ...
              <div className="bg-indigo-50/40 p-4 rounded-xl border border-indigo-100">
                ...
                <form onSubmit={handleUpdateDetail} className="space-y-3">
                  <div className="grid grid-cols-1 gap-3">
                    <div>
                      <label className="block font-semibold text-slate-600 mb-1">Status Alur Kerja Saat Ini</label>
              ...
              <div className="mt-2 border-t border-slate-100 pt-4">
                <div className="p-5 rounded-xl border-2 border-dashed border-indigo-200 bg-indigo-50/20 flex flex-col items-center text-center">
                  {selectedSuratData.file_surat ? (
                    <div className="w-full">
                      ...
                      {selectedSuratData.file_surat.match(/\.(jpeg|jpg|gif|png|webp)$/i) ? (
                        <img src={selectedSuratData.file_surat} alt="Preview" className="w-full h-auto rounded border border-slate-200" />
                      ) : (
                        <div className="p-4 bg-white border border-slate-200 rounded flex flex-col items-center">
                          ...
                          <a href={selectedSuratData.file_surat} target="_blank" rel="noopener noreferrer" className="mt-2 px-4 py-1.5 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 rounded-md font-semibold text-xs border border-indigo-100">Buka / Unduh Berkas</a>
                        </div>
                      )}
                    </div>
                  ) : (
            ...
            <div className="pt-4 border-t border-slate-100 flex justify-between items-center">
              <button 
                ...
                className="text-indigo-600 hover:text-indigo-700 text-xs font-semibold flex items-center space-x-1 px-4 py-2 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition"
              >
                <ExternalLink className="w-3.5 h-3.5" />
                <span>Buka di Generator A4</span>
              </button>
              <button type="button" onClick={() => setIsDetailModalOpen(false)} className="px-5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg font-semibold text-xs transition">
                Tutup Detail
              </button>
            </div>
          </div>
